...
const HantlePlasko = () => {
  ...
  const Create = () => {
    ...
    if (first && last && repeat) {
      ...
      addDoc(collection(db, "wegithlifting_hantle_flat"), docData)
        .then(() => {
  ...
  const Read = () => {
    const myCollection = collection(db, "wegithlifting_hantle_flat");
  ...
  const Delete = (id) => {
    const docRef = doc(db, "wegithlifting_hantle_flat", id);
